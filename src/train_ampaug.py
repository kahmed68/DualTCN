"""
train_p9d_ampaug.py — Retrain DualTCN (P9d) with amplitude-channel augmentation.

Revised with three improvements:
  1. Narrower augmentation: σ_amp in [0.001, 0.01] log10 (~0.2-2%)
  2. Curriculum: first 20 epochs clean, then ramp augmentation 0→1 over
     epochs 20-40, full strength from epoch 40 onward.
  3. Tighter gradient clipping (max_norm=0.5) to fix inf gradient norms.

After training, runs the amplitude noise evaluation on the new model.
"""
import os
import sys
import numpy as np
...
from dataset_v4_ampaug import get_dataloaders_v4_ampaug
...
MODEL_PATH  = "best_model_p9d_ampaug.pt"
RESUME_PATH = "resume_p9d_ampaug.pt"
CURVE_PATH  = "training_curve_p9d_ampaug.png"
PAPER_DIR   = os.path.join(DIR, "paper")


# ── Amplitude noise evaluation (reused from amplitude_noise_experiment.py) ────

def evaluate_amplitude_robustness(model_path, device):
    """Run amplitude perturbation experiments on the trained model."""
    ...
    print("\n" + "=" * 60)
    print("Amplitude noise evaluation on amp-augmented model")
    print("=" * 60)
    ...
    @torch.no_grad()
    def predict_with_amp_perturbation(E, A, Pr, P, amp_noise_std=0.0,
                                       amp_bias=0.0):
        """Predict with perturbed amplitude channels."""
        ...
        for x, _, _ in loader:
            x = x.to(device)
            # Perturb amplitude channels (indices 1, 3, 5, 7)
            if amp_noise_std > 0:
                for ch in [1, 3, 5, 7]:
                    noise = torch.randn_like(x[:, ch, :]) * amp_noise_std
                    x[:, ch, :] = x[:, ch, :] + noise
            if amp_bias != 0:
                for ch in [1, 3, 5, 7]:
                    x[:, ch, :] = x[:, ch, :] + amp_bias
    ...
    # Clean baseline
    pred_clean = predict_with_amp_perturbation(E_test, A_test, Pr_test, P_test)
    ...
    for sigma_amp in [0.00, 0.01, 0.02, 0.05, 0.10, 0.20]:
        pred = predict_with_amp_perturbation(
    ...
    for fname, rows, fields in [
        ("ampaug_random_noise.csv", rows_random,
         ["sigma_amp", "r2_sigma1", "r2_sigma2", "r2_d1", "r2_d2", "r2_mean"]),
        ("ampaug_systematic_bias.csv", rows_bias,
...
def train_one_epoch_p9d_amp(model, loader, optimizer, criterion, scheduler,
    ...
    """Like train_one_epoch_p9d but with AMP mixed-precision."""
    model.train()
    ...
    for x, sig, p_true in loader:
        ...
        # Tighter gradient clipping to prevent inf norms
...
def run_training_p9d_amp(model, train_loader, val_loader, test_loader,
    ...
    """Like run_training_p9d but with AMP mixed-precision."""
    start_epoch = 1
    best_val    = float("inf")
    ...
    # Curriculum schedule: clean for first 20 epochs, ramp 0→1 over 20-40
    CURRICULUM_START = 20
    CURRICULUM_END   = 40

    for epoch in range(start_epoch, n_epochs + 1):
        # Set curriculum scale on training dataset
...
def main():
    ...
    print(f"Experiment: DualTCN ampaug v2 (narrower range + curriculum)")
    print(f"  Amplitude noise: σ_amp in [0.001, 0.01] log10 (~0.2-2%)")
    ...
    train_loader, val_loader, test_loader = get_dataloaders_v4_ampaug(
    ...
    print(f"LateTimePCRN (P9d-AmpAug) parameters: {n_par:,}")
